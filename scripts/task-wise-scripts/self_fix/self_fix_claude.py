...
class ClaudeAdapter:
    ...
    def __init__(self, model: str = "claude-sonnet-4-5", timeout: int = 300, rate_limit_delay: float = 1.0):
        self.model = model
    ...
    def _convert_messages(self, messages: list) -> Tuple[Optional[str], list]:
        ...
        for m in messages:
            ...
            if role == "system":
                ...
                conv_messages.append({"role": anthropic_role, "content": str(content)})

        system_prompt = "\n\n".join(system_parts).strip() if system_parts else None
        return system_prompt, conv_messages
    ...
    def generate(self, messages: list) -> Optional[str]:
        ...
        try:
            ...
            system_prompt, conv_messages = self._convert_messages(messages)

            print(f"  -> Sending code to Claude model ({self.model})...", flush=True)
            ...
            print(f"  -> Content length: {len(response_text) if response_text else 0}", flush=True)
            ...
            print(f"  -> Warning: Claude API error: {type(e).__name__}: {e}", flush=True)
            import traceback
            traceback.print_exc()
            return None
        except Exception as e:
            print(f"  -> Warning: Claude request failed: {type(e).__name__}: {e}", flush=True)
            import traceback
            traceback.print_exc()
            return None
...
def process_folder(
    ...
    adapter = ClaudeAdapter(model=model, rate_limit_delay=rate_limit_delay)
        ...
        try:
            ...
            if os.path.exists(history_file_path):
                try:
                    ...
                    history_log = existing.get("history", []) or []
                    attempts_done = len(history_log)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run iterative code generation with Claude Sonnet.")
    ...
    parser.add_argument("--output_dir", type=str, default="./claude_outputs", help="Output directory for results")
    parser.add_argument("--model", type=str, default="claude-sonnet-4-5", help="Claude model to use (default: claude-sonnet-4-5)")
    ...
    print("Starting Claude Sonnet Self-Fixing Code Generation")
