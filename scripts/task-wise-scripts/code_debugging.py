...
def main():
    parser = argparse.ArgumentParser(description="Debug Python code using a specified model.")
    ...
    model_output_dir = os.path.join(output_dir, model_name, "python3")
    os.makedirs(model_output_dir, exist_ok=True)
    output_file = os.path.join(model_output_dir, f"{model_name}_python3_results.json")
    checkpoint_file = os.path.join(model_output_dir, f"{model_name}_python3_checkpoint.json")
    ...
    for idx, item in enumerate(dataset, 1):
        ...
        if item.get("code_language") == "python3":
            ...
            system_prompt = "You are an expert at debugging Python code."
...
{incorrect_solution}
```
Provide only the corrected and complete Python code inside a markdown block. Do not provide any explanation."""
            ...
            if not debugged_code:
                print(f"Warning: No Python code block found for item {item_idx}. Recording as 'no response'.", flush=True)
                results.append({
                    "id": item.get("idx"),
                    "status": "no response",
            ...
            try:
                ...
                print(f"  Warning: Could not calculate cyclomatic complexity: {e}", flush=True)
                cc = -1
            ...
            try:
                ...
                print(f"  Warning: Could not calculate Halstead metrics: {e}", flush=True)
                halstead = -1
            ...
            try:
                ...
                print(f"  Warning: Could not calculate cognitive complexity: {e}", flush=True)
                cognitive_complexity = -1
            ...
            code_smells = False
            with tempfile.NamedTemporaryFile(mode='w+', suffix='.py', delete=True) as temp_f:
