...
def run_tests(code, test_cases):
    ...
    try:
        compile_process = subprocess.run(
            ["g++", "-o", executable_path, source_file_path],
            capture_output=True,
            text=True,
            timeout=10
        )
...
def main():
    parser = argparse.ArgumentParser(description="Debug C++ code using a specified model.")
    ...
    model_output_dir = os.path.join(output_dir, model_name, "cpp")
    os.makedirs(model_output_dir, exist_ok=True)
    output_file = os.path.join(model_output_dir, f"{model_name}_cpp_results.json")
    checkpoint_file = os.path.join(model_output_dir, f"{model_name}_cpp_checkpoint.json")
    ...
    for idx, item in enumerate(dataset, 1):
        ...
        if item.get("code_language") == "cpp":
            ...
            system_prompt = "You are an expert at debugging C++ code."
...
{incorrect_solution}
```
Provide only the corrected and complete C++ code inside a markdown block. Do not provide any explanation."""
            ...
            if not debugged_code:
                print(f"Warning: No C++ code block found for item {item_idx}. Recording as 'no response'.", flush=True)
                results.append({
                    "id": item.get("idx"),
                    "status": "no response",
            ...
            with tempfile.NamedTemporaryFile(mode='w+', suffix='.cpp', delete=True) as temp_f:
