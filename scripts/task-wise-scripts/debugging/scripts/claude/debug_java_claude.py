...
def main():
    parser = argparse.ArgumentParser(description="Debug Java code using Claude Sonnet 4.5.")
    ...
    checkpoint_file = "Claude-Sonnet-4.5_java_checkpoint.json"
    output_file = "Claude-Sonnet-4.5_java_results.json"
...
Corrected code:"""
            ...
            if not debugged_code:
                print(f"Warning: No Java code block found in response for item {item_idx}.")
                results.append({
                    "id": item.get("idx"),
                    "status": "no response",
                    "pass_rate": 0,
