...
def main():
    parser = argparse.ArgumentParser(description="Debug C++ code using GPT-5.")
    ...
    print(f"Using {model_name} for C++ Debugging")

    checkpoint_file = f"{model_name}_cpp_checkpoint.json"
    output_file = f"{model_name}_cpp_results.json"
...
Corrected code:"""
            ...
            if not code_match: code_match = re.search(r"```c\+\+\n(.*?)```", response_text, re.DOTALL)
            ...
            if not debugged_code:
                print("Warning: No C++ code block found.")
                results.append({"id": item.get("idx"), "status": "no response", "pass_rate": 0})
                continue
            ...
            codebleu_value = -1
            try:
                cb = codebleu.calc_codebleu([correct_solution], [debugged_code], lang="cpp", weights=(0.25, 0.25, 0.25, 0.25))
